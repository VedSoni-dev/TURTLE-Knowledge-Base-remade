...
- **Barrel Connectors** - most common on commercial electronics. Can be very proprietary in size.
- **Anderson Powerpole Connectors** - FIRST Robotics standard connector. Universally genderless design, color coded for various outputs.
- **4mm Bullet Connectors** - common for smaller ESC connections. Have a smaller profile than other high-current connectors.

> **⚠️ Warning:** USB ports on microcontroller boards are not designed for high current (limited to ~200mA). Avoid using USB ports for power when possible.
...
> **Note:** There are plenty of open source communication protocols (SPI, RS232, RS485, etc), as well as various proprietary protocols. Industry practice considers EMI, cybersecurity, and speed concerns, but the ones listed above are very well documented and designed for modularity.
